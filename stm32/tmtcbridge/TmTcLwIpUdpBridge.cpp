...
ReturnValue_t TmTcLwIpUdpBridge::udp_server_init(void) {
	...
	if (TmTcLwIpUdpBridge::upcb)
	{
		...
		err = udp_bind(TmTcLwIpUdpBridge::upcb, IP_ADDR_ANY, UDP_SERVER_PORT); // @suppress("Invalid arguments") // @suppress("Symbol is not resolved")
...
ReturnValue_t TmTcLwIpUdpBridge::receiveTc(uint8_t ** recvBuffer, size_t * size) {
	*size = 0;
	return RETURN_OK;
}
...
ReturnValue_t TmTcLwIpUdpBridge::sendTm(const uint8_t * data, size_t dataLen) {
	...
	if ((p_tx != NULL) && (lastAdd.addr != IPADDR_TYPE_ANY) && (upcb != NULL)) {
		...
		err = udp_connect(TmTcLwIpUdpBridge::upcb, &lastAdd, UDP_CLIENT_PORT); // @suppress("Symbol is not resolved") // @suppress("Invalid arguments")
		if(err != ERR_OK){
			pbuf_free(p_tx);
			return err;
		}
...
void TmTcLwIpUdpBridge::udp_server_receive_callback(void* arg,
	...
	if(p_tx != NULL)
	{
		...
		*(data+p_tx->len) = '\0';
		...
		// bytearray with one entry 0 means disconnected client for now
		if(p->len == 5 && data[0] == 0) {
			udpBridge->registerCommDisconnect();
			return;
		}

		sif::info << "UDP Server: Received data" << std::endl;
