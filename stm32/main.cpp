/**
 * \file main.cpp
 * Entry point for STM32 dev-board code. Starts the FreeRTOS scheduler
 * and initializes the mission specific code by calling init_mission()
 *
 * \defgroup board Board specific code
 */
...
int main(void)
{
  ...
  /* UART3 Config with baudrate 115200 for debug output */
  MX_USART3_UART_Init(115200);

  BSP_Config();

  xTaskCreate( initTask, "init", 2048, NULL, 1, NULL); // @suppress("Invalid arguments")
...
static void Netif_Config(void)
{
	...
	netif_add(&gnetif, &ipaddr, &netmask, &gw, NULL, &ethernetif_init, &ethernet_input); // @suppress("Invalid arguments")
