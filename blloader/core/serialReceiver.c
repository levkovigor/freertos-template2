...
#include <blloader/core/serialReceiver.h>
...
/* Private variables */
// Propably should use a ring buffer..
#define RECEPTION_BUFFER_SIZE 1024
uint8_t reception_buffer[2][RECEPTION_BUFFER_SIZE];
uint8_t* current_buffer = NULL;
SemaphoreHandle_t uartMutex;
bool buffer_0_full = false;
bool buffer_1_full = false;

size_t buffer_0_bytes_received = 0;
size_t buffer_1_bytes_received = 0;
...
void configure_usart0(uint32_t baud_rate, uint16_t timeout) {
    unsigned int mode = AT91C_US_USMODE_NORMAL
        ...
        uartConfig.rxtimeout = timeout;
        uartConfig.timeGuard = 0;
        UART_start(bus0_uart, uartConfig);
