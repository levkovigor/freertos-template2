...
void taskUARTtest(void *arguments) {
	...
	while(1) {
		...
		retValInt = UART_write(bus, wrData, 6);
...
//	UTIL_DbguGetIntegerMinMax(&bus2type, 0, 1);
	...
	if(retValInt != 0) {
		TRACE_WARNING("\n\r UARTtest: UART_start returned %d! bus 0 \n\r", retValInt);
		while(1);
	}
	retValInt = UART_start(bus2_uart, configBus2);
	if(retValInt != 0) {
		TRACE_WARNING("\n\r UARTtest: UART_start returned %d! bus 1 \n\r", retValInt);
		while(1);
	}

	// Instantiate two separate versions of taskUARTtest and pass different bus-id's as a parameter.
	xTaskGenericCreate(taskUARTtest, (const char*)"taskUARTtest-0", 1024, (void*)&UARTtestBus[0], 2, &taskUART0testHandle, NULL, NULL);
	xTaskGenericCreate(taskUARTtest, (const char*)"taskUARTtest-2", 1024, (void*)&UARTtestBus[1], 2, &taskUART2testHandle, NULL, NULL);
