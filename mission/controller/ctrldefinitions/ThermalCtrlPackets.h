...
public:
    ...
    lp_var_t<float> tempTestHkb = lp_var_t<float>(
            ThermalCtrlPoolIds::TEMP_TEST_HKB,
            this->hkManager->getOwner(),
            this);
    // todo: add all PT1000s temperatures here.
    lp_var_t<float> tempSyrlinks = lp_var_t<float>(
            ThermalCtrlPoolIds::TEMP_SYRLINKS,
            this->hkManager->getOwner(), this);
    lp_var_t<float> tempMeshcam = lp_var_t<float>(
            ThermalCtrlPoolIds::TEMP_MESHCAM,
            this->hkManager->getOwner(), this);
    lp_var_t<float> tempCcsds = lp_var_t<float>(
            ThermalCtrlPoolIds::TEMP_CCSDS,
            this->hkManager->getOwner(), this);
    lp_var_t<float> tempPloc = lp_var_t<float>(
            ThermalCtrlPoolIds::TEMP_PLOC,
            this->hkManager->getOwner(), this);
