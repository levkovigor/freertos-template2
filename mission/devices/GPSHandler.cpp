...
void GPSHandler::doStartUp() {
	...
	case InternalState::STATE_WAIT_FIRST_MESSAGE:
		sif::info << "GPS Handler: Waiting for first message" << std::endl;
	...
	case InternalState::STATE_SET_MESSAGE_TYPE: {
		if (commandExecuted) {
			sif::info << "GPS Handler: Configuration successfull! "
			        "Switching On" << std::endl;
...
ReturnValue_t GPSHandler::buildTransitionDeviceCommand(DeviceCommandId_t* id) {
	...
	case InternalState::STATE_CONFIGURED:
		return DeviceHandlerBase::NOTHING_TO_SEND;
	case InternalState::STATE_SET_UPDATE_RATE: {
		sif::info << "GPS Handler: Building a SET_GPS_UPDATE_RATE "
		        "command now" << std::endl;
	...
	case InternalState::STATE_SET_MESSAGE_TYPE: {
		sif::info << "GPS Handler: Building a SET_GPS_MESSAGE_TYPE "
		        "command now" << std::endl;
...
ReturnValue_t GPSHandler::checkBinaryReply(const uint8_t *start, size_t * len,
	...
	case ACK:
		...
		*foundId = start[PAYLOAD_START_INDEX + 1];
		sif::info << "GPS Handler: Got reply " << std::hex
		        << *foundId << std::endl;
		size = 2 + BINARY_HEADER_AND_TAIL_SIZE;
		break;
	default:
		* foundLen = 1;
		sif::info << "GPSHandler::checkBinaryReply: Ignoring trash because "
		        "start ID is weird" << std::endl;
		return IGNORE_FULL_PACKET;
	}
	...
	if(foundChecksum != calculatedChecksum) {
		sif::error << "GPS Handler: Checksum error !" << std::endl;
